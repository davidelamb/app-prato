...
const empty: Player = { id: '', name: '', role: 'Attaccante', appearances: 0, goals: 0, assists: 0, source: 'Editoriale', imageScale: 1, imagePositionY: 0 };
...
export function PlayersAdmin({ content, onChange }: { content: AppContent; onChange: (next: AppContent) => Promise<void> }) {
  ...
  return <View style={{ gap: 14 }}>
    <View style={adminStyles.panel}>
      ...
      <View style={[adminStyles.preview, { overflow: 'hidden' }]}>{draft.imageUrl ? <Image source={{ uri: draft.imageUrl }} resizeMode="cover" style={[adminStyles.previewImage, playerImageStyle(draft)]} /> : <MaterialCommunityIcons name="account" size={60} color={colors.mutedDark} />}</View>
    ...
    <View style={adminStyles.panel}>
      ...
      <View style={adminStyles.list}>{content.players.map((player) => <Pressable key={player.id} onPress={() => { setEditing(player.id); setDraft({ ...player, imageScale: player.imageScale ?? 1, imagePositionY: player.imagePositionY ?? 0 }); }} style={adminStyles.listRow}>
