...
export function LiveAdmin({ content, onChange }: { content: AppContent; onChange: (next: AppContent) => Promise<void> }) {
  ...
  return <View style={{ gap: 14 }}>
    ...
    <View style={adminStyles.panel}><Text style={adminStyles.title}>Registra gol AC Prato</Text><View style={adminStyles.row}><Field label="Marcatore" value={scorer} onChangeText={setScorer} /><Field label="Minuto" value={minute} onChangeText={setMinute} keyboardType="numeric" /></View><Button label="Aggiungi gol" icon="soccer" onPress={goal} /></View>
